...
- `AmmoProfileDefinition` declara `round_weight_kg` obligatorio, finito y estrictamente mayor que cero como masa física canónica por round, además de impacto médico determinista mediante `wound_type`, `wound_severity`, `bleeding_rate_per_game_hour`, `pain_contribution` y `penetration_power`. El runtime traduce el impacto a M39/M40.1 y deriva la masa interna de una firearm como `LoadedRounds × round_weight_kg`; no declara scripts ni un daño paralelo por arma.
- Todo `ItemDefinition` con `ammo_profile_id` representa rounds loose y debe tener `physical.weight_kg` finito, mayor que cero y coincidente con el `round_weight_kg` del profile dentro de `0.000001 kg`. Divergir es error de validación; no existe corrección silenciosa ni selección del primer item compatible. Varios items namespaced pueden referenciar el mismo profile si todos respetan su masa canónica.
...
- El estado cargado no vive en JSON de definitions: cada `ItemInstance` firearm mantiene ammo profile y rounds en runtime/save; capacity siempre deriva del firearm profile y la masa siempre deriva del ammo profile actual. El save no duplica masa ni requiere migración de schema por este contrato.
