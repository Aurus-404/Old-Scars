...
Estado: `MACRO ELEVATION / LANDFORMS V1 CLOSED`.
...
La shell implementada posee session lifecycle único, save catalog, Main Menu, World Runtime placeholder e in-game Save/Return. New Game genera un plan finito completo y después un campo mundial fixed-point de elevation/landforms. `world_session_v1` schema `3` persiste plan + geography committed sobre M37 y mantiene compatibilidad explícita con schemas `1`/`2` sin fabricar truth. `current_slice_v1` permanece intacto y provenance se guarda sólo como evidencia.

El próximo coding unit candidato es `ID TBD — Macro Hydrology / Coastlines V1`, con estado `PLANNED — NOT AUTHORIZED`. Debe consumir MacroWorldPlan + MacroGeography como verdad global y agregar únicamente water/coastline truth expresamente autorizada. No autoriza terrain, materialización, climate, geology, vegetation/biomes, roads, settlements, history, sector transitions, generation compatibility ni gameplay world simulation.

Fuera de alcance mientras no exista autorización específica:

- cualquier implementación open-world posterior a Macro Elevation / Landforms V1 sin autorización del siguiente unit;
- M42.0 u otro milestone jugable;
- cambios de gameplay, content contracts o persistencia fuera del payload schema `3` acotado ya cerrado;
...
- Macro Hydrology / Coastlines V1, otros coding units open-world o M42.0 sin autorización específica;
...
- facciones amplias;
- hydrology/coastlines, climate, geology, vegetation/biomes, sectors jugables, transición, world history o gameplay world persistence antes de sus units autorizadas;
