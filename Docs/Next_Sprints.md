...
Estado: `MACRO WORLD PLAN V1 CLOSED`.
...
La shell implementada posee session lifecycle único, save catalog, Main Menu, World Runtime placeholder e in-game Save/Return. New Game ya reemplazó el starter sector por un plan finito completo con size seleccionado, settings resueltos, bounds, placements y topology; `world_session_v1` schema `2` persiste esa verdad sobre M37 y mantiene compatibilidad explícita con schema `1`. `current_slice_v1` permanece intacto y provenance se guarda sólo como evidencia.

El próximo coding unit candidato es `ID TBD — Macro Elevation / Landforms V1`, con estado `PLANNED — NOT AUTHORIZED`. Debe consumir el MacroWorldPlan validado y agregar únicamente los campos/landforms expresamente autorizados. No autoriza terrain, materialización, hydrology/oceans, roads, settlements, history, sector transitions, generation compatibility ni gameplay world simulation.

Fuera de alcance mientras no exista autorización específica:

- cualquier implementación open-world posterior a Macro World Plan V1 sin autorización del siguiente unit;
- M42.0 u otro milestone jugable;
- cambios de gameplay, content contracts o persistencia fuera del payload schema `2` acotado ya cerrado;
...
- Macro Elevation / Landforms V1, otros coding units open-world o M42.0 sin autorización específica;
...
- facciones amplias;
- elevation/landforms, geography, sectors jugables, transición, world history o gameplay world persistence antes de sus units autorizadas;
