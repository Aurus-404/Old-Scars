...
- Climate, moisture, rivers, geology, vegetation/biomes, roads, sites, history, terrain/materialization y workers continúan pendientes. No existe selector de workers: primero se midió el workload y la futura ejecución paralela deberá conservar exactamente el mismo output con `1` o `N` workers.
...
- `WorldSession` es dato lógico inmutable separado de scenes y filesystem. Los mundos nuevos contienen `WorldId`, display name, `WorldGenerationContext`, `MacroWorldPlan`, `MacroGeographyPlan`, `MacroWaterPlan`, quality analysis derivada, `ActiveSectorId` y evidencia del `LoadedContentSet` capturada al crear el mundo; `Topology` se deriva del plan. El display name no participa en identidad ni ruta de save.
- `WorldSessionBootstrap` usa `GeneratorVersion = world_pipeline_v2` como metadata global de creación y ejecuta plan → geografía → Water → quality → starter → session. Plan/Geography conservan sus propios contratos deterministas y Water usa su contrato/inputs de pass, por lo que un cambio downstream no re-seedea outputs upstream sin una dependencia explícita. Los persistidos nunca regeneran truth desde seed. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo contratos deterministas y nunca bajo `UnityEngine.Random`.
- El snapshot type continúa siendo `world_session_v1`; los mundos nuevos usan schema `4`. El payload lógico hermano dentro del envelope M37 persiste generation context, plan, geography, settings/sea/masks/bodies/coastline/conditioned surface/drainage/basins/hash de Water, active sector y provenance por source/set. Quality se reconstruye desde truth committed durante preflight. Usa `PersistenceSerializer` y `PersistenceFileStore`; el slot canónico es `WorldId.Canonical`, no el display name. No modifica ni reinterpreta `current_slice_v1`.
- Schemas `1`, `2` y `3` conservan paths legacy explícitos y delimitados. Schema `1` reconstruye topología/active sector sin inventar preset, plan, geografía o Water; schema `2` reconstruye MacroWorldPlan sin fabricar geography; schema `3` reconstruye su MacroWorldPlan + MacroGeography sin fabricar Water ni quality truth. Si se vuelven a guardar, permanecen en su schema original.
- La lectura schema `4` ejecuta deserialize estricto, parseo de IDs, reconstrucción mediante validators reales de settings/bounds/`WorldTopology`/`MacroWorldPlan`/`MacroGeographyPlan`/`MacroWaterPlan`, verificación de hashes/lengths/drainage/basins, membership del active sector, quality hard requirements y provenance estructural antes de publicar. Geography/Water se rehidratan desde truth committed, no regenerando desde seed/código actual. Provenance no decide generation/save compatibility.
