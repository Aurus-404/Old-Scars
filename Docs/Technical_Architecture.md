...
- No existen todavía logical pose, climate/moisture, rivers, history, terrain, materialización, sector streaming/transitions, active-sector simulation ni GameObjects mundiales. `current_slice_v1` permanece sin cambios.
...
- Climate, moisture, rivers, geology, vegetation/biomes, settlements detallados, history, terrain/materialization y workers continúan pendientes. La infraestructura vial macro V1 se documenta abajo; no materializa carreteras ni declara calles, puentes o navegación física. No existe selector de workers: primero se midió el workload y la futura ejecución paralela deberá conservar exactamente el mismo output con `1` o `N` workers.
...
- `WorldSession` es dato lógico inmutable separado de scenes y filesystem. Los mundos nuevos contienen `WorldId`, display name, `WorldGenerationContext`, `MacroWorldPlan`, `MacroGeographyPlan`, `MacroWaterPlan`, quality analysis derivada, `MacroHumanGeographyPlan`, `ActiveSectorId` y evidencia del `LoadedContentSet` capturada al crear el mundo; `Topology` se deriva del plan. El display name no participa en identidad ni ruta de save.
- `WorldSessionBootstrap` usa `GeneratorVersion = world_pipeline_v3` como metadata global de creación y ejecuta plan → geografía → Water → quality → starter → Human Geography → session. Cada pass conserva su propio contrato determinista, por lo que un cambio downstream no re-seedea outputs upstream sin una dependencia explícita. Los persistidos nunca regeneran truth desde seed. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo contratos deterministas y nunca bajo `UnityEngine.Random`.
- El snapshot type continúa siendo `world_session_v1`; los mundos nuevos usan schema `5`. El payload lógico hermano dentro del envelope M37 persiste generation context, plan, geography, Water, Human Geography con settings/sites/roads/polylines/hash, active sector y provenance por source/set. Quality se reconstruye desde truth committed durante preflight. Usa `PersistenceSerializer` y `PersistenceFileStore`; el slot canónico es `WorldId.Canonical`, no el display name. No modifica ni reinterpreta `current_slice_v1`.
- Schemas `1`, `2`, `3` y `4` conservan paths legacy explícitos y delimitados. Schema `1` no inventa plan/geography/Water/Human Geography; schema `2` no fabrica geography posterior; schema `3` no fabrica Water/quality; schema `4` conserva Plan + Geography + Water sin fabricar Human Geography. Si se vuelven a guardar, permanecen en su schema original.
- La lectura schema `5` ejecuta deserialize estricto, parseo de IDs, reconstrucción mediante validators reales de Plan/Geography/Water/Human Geography, verificación de hashes, endpoints, land components, polylines/cost metadata, active sector, quality hard requirements y provenance estructural antes de publicar. Toda truth se rehidrata committed, no regenerando desde seed/código actual. Provenance no decide generation/save compatibility.
...
- M41.1 agrega el bloque opcional data-driven `encounter_ai` y `HumanEncounterAIController`. Este último sólo posee target explícitamente asignado, state/timers y response `avoid`/`flee`/`fight`; delega LOS en `ActorVisualPerceptionService`, path en `ActorNavigationController` y ammo/reload/impacto en `WeaponCombatService`.
- `LostContact` conserva sólo `LastKnownPosition` de una percepción positiva, no consulta la posición del target oculto, cancela acción activa y limpia el encounter tras timeout; una reacquisition exige asignación explícita. `Dead` detiene navegación y acciones.
