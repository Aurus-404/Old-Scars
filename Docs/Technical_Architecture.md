...
- No existen todavía logical pose, macro geography, history, terrain, materialización, sector streaming/transitions, active-sector simulation ni GameObjects mundiales. `current_slice_v1` permanece sin cambios.
...
- `WorldSession` es dato lógico inmutable separado de scenes y filesystem. Contiene `WorldId`, display name, `WorldGenerationContext`, `WorldTopology`, `ActiveSectorId` y evidencia del `LoadedContentSet` capturada al crear el mundo. El display name no participa en identidad ni ruta de save.
- `WorldSessionBootstrap` es un reemplazo temporal y explícito del macro world plan todavía inexistente. Usa `GeneratorVersion = bootstrap_v1`, el pass determinista existente `topology/starter_sector` y crea exactamente un `SectorId` en una topología válida de un nodo. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo `WorldDeterminism` y nunca bajo `UnityEngine.Random`.
- `world_session_v1`, schema `1`, es un payload lógico hermano dentro del envelope M37 existente. Usa `PersistenceSerializer` y `PersistenceFileStore`; el slot canónico es `WorldId.Canonical`, no el display name. Persiste seed/version, topología completa y hash canónico existente, active sector y evidencia de provenance por source/set. No modifica ni reinterpreta `current_slice_v1`.
- La lectura ejecuta deserialize estricto, parseo de cada ID, reconstrucción mediante `WorldTopology.TryCreate`, verificación de `CanonicalHash`, membership del active sector y validación estructural de provenance antes de publicar. La evidencia de creación no se compara con el contenido actual y no decide generation/save compatibility.
...
- El flujo de producto es `MainMenu → WorldRuntime`. `MainMenu` contiene `GameDataManager` y New Game/Load Game/Exit; Create debe persistir antes de entrar al runtime. `WorldRuntime` es un placeholder separado de `SampleScene` y sólo demuestra una session validada; Escape abre Continue/Save/Return/Exit. Return cierra la session sin autosave implícito. `SampleScene` permanece como laboratorio de regressions y continúa en Build Settings.
- Esta shell no persiste gameplay/sector state, no aplica `CurrentSliceLoadService`, no implementa macro worldgen y no completa la futura World Persistence de producción.
