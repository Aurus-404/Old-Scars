...
- `MacroWorldPlanGenerator` consume `WorldGenerationContext + WorldGenerationSettings`. Deriva IDs y candidates desde `WorldDeterminism`, elige placements mediante maximin determinista y construye un árbol espacial mínimo como `WorldTopology` conectada. Ese MST es sólo el scaffold lógico de conectividad V1: no es physical adjacency, road graph ni travel graph. No usa `WorldId`, `UnityEngine.Random`, `System.Random`, `GetHashCode`, grilla rígida ni insertion/filesystem order.
...
- `MacroGeographyGenerator` consume `WorldGenerationContext + MacroWorldPlan` después del esqueleto mundial. Deriva una vez seis domains aislados (`landform_regions`, `regional_upheaval`, `base_elevation`, `relief_detail`, `mountain_ridges`, `surface_roughness`) mediante `WorldDeterminism`; el inner loop usa value noise/fBm fixed-point y un mixer numérico explícito, no SHA-256 por sample, `Mathf.PerlinNoise`, `UnityEngine.Random`, `System.Random` ni runtime hashes.
...
- `WorldSessionBootstrap` usa `GeneratorVersion = macro_water_quality_v1` y ejecuta plan → geografía → Water → quality → starter → session. El cambio de contrato afecta sólo nuevos mundos; los persistidos nunca regeneran truth desde seed. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo contratos deterministas y nunca bajo `UnityEngine.Random`.
