...
- No existen todavía logical pose, macro geography más allá de elevation/landforms, history, terrain, materialización, sector streaming/transitions, active-sector simulation ni GameObjects mundiales. `current_slice_v1` permanece sin cambios.
...
- `MacroWorldPlanGenerator` consume `WorldGenerationContext + WorldGenerationSettings`. Deriva IDs y candidates desde `WorldDeterminism`, elige placements mediante maximin determinista y construye un árbol espacial mínimo como `WorldTopology` conectada. No usa `WorldId`, `UnityEngine.Random`, `System.Random`, `GetHashCode`, grilla rígida ni insertion/filesystem order.
- Una conexión V1 significa sólo relación regional topológica. No representa road, river, railway, crossing geometry ni dirección; la capacidad existente de múltiples connection keys entre el mismo par continúa disponible.
...
- Climate, moisture, hydrology/coastlines, geology, vegetation/biomes, roads, sites, history, terrain/materialization y workers continúan pendientes. No existe selector de workers: primero se midió el workload y la futura ejecución paralela deberá conservar exactamente el mismo output con `1` o `N` workers.
...
- `WorldSession` es dato lógico inmutable separado de scenes y filesystem. Los mundos actuales contienen `WorldId`, display name, `WorldGenerationContext`, `MacroWorldPlan`, `MacroGeographyPlan`, `ActiveSectorId` y evidencia del `LoadedContentSet` capturada al crear el mundo; `Topology` se deriva del plan. El display name no participa en identidad ni ruta de save.
- `WorldSessionBootstrap` usa `GeneratorVersion = macro_geography_v1`, genera plan y geografía global antes de publicar la session, y elige como active sector el placement más central. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo `WorldDeterminism` y nunca bajo `UnityEngine.Random`.
- El snapshot type continúa siendo `world_session_v1`; los mundos nuevos usan schema `3`. El payload lógico hermano dentro del envelope M37 persiste generation context, plan, samples/resolved settings/hash de macro geography, active sector y provenance por source/set. Usa `PersistenceSerializer` y `PersistenceFileStore`; el slot canónico es `WorldId.Canonical`, no el display name. No modifica ni reinterpreta `current_slice_v1`.
- Schemas `1` y `2` conservan paths legacy explícitos y delimitados. Schema `1` reconstruye topología/active sector sin inventar preset, plan o geografía; schema `2` reconstruye su MacroWorldPlan sin fabricar elevation/landforms. Si se vuelven a guardar, permanecen en su schema original.
- La lectura schema `3` ejecuta deserialize estricto, parseo de IDs, reconstrucción mediante validators reales de settings/bounds/`WorldTopology`/`MacroWorldPlan`/`MacroGeographyPlan`, verificación de hashes y sample lengths, membership del active sector y validación estructural de provenance antes de publicar. La geografía se rehidrata desde samples committed, no regenerando desde seed/código actual. Provenance no decide generation/save compatibility.
...
- El flujo de producto es `MainMenu → WorldRuntime`. `MainMenu` contiene `GameDataManager` y New Game/Load Game/Exit; New Game permite nombre, seed opcional y size con default `Large`, y debe generar/persistir antes de entrar al runtime. `WorldRuntime` es un placeholder separado de `SampleScene` y sólo demuestra una session validada; Escape abre Continue/Save/Return/Exit. Return cierra la session sin autosave implícito. `SampleScene` permanece como laboratorio de regressions y continúa en Build Settings.
- Esta shell no persiste gameplay/sector state, no aplica `CurrentSliceLoadService` y no completa hydrology/climate, local detail ni la futura World Persistence de producción.
