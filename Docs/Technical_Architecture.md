...
- No existen todavía logical pose, macro geography más allá de placement/topology, history, terrain, materialización, sector streaming/transitions, active-sector simulation ni GameObjects mundiales. `current_slice_v1` permanece sin cambios.
...
- `WorldSession` es dato lógico inmutable separado de scenes y filesystem. Los mundos actuales contienen `WorldId`, display name, `WorldGenerationContext`, `MacroWorldPlan`, `ActiveSectorId` y evidencia del `LoadedContentSet` capturada al crear el mundo; `Topology` se deriva del plan. El display name no participa en identidad ni ruta de save.
- `WorldSessionBootstrap` usa `GeneratorVersion = macro_plan_v1`, genera el plan completo y elige como active sector el placement más central. Un seed vacío sólo selecciona un `WorldSeed` mediante `RandomNumberGenerator`; toda derivación posterior continúa bajo `WorldDeterminism` y nunca bajo `UnityEngine.Random`.
- El snapshot type continúa siendo `world_session_v1`; los mundos nuevos usan schema `2`. El payload lógico hermano dentro del envelope M37 persiste generation context, settings resueltos, bounds, placements, topology/hash, plan hash, active sector y provenance por source/set. Usa `PersistenceSerializer` y `PersistenceFileStore`; el slot canónico es `WorldId.Canonical`, no el display name. No modifica ni reinterpreta `current_slice_v1`.
- Schema `1` conserva un path legacy explícito y delimitado: reconstruye su topología/active sector original sin inventar preset, bounds o plan. Si se vuelve a guardar, permanece schema `1`; no se reinterpreta silenciosamente como un mundo Macro Plan V1.
- La lectura schema `2` ejecuta deserialize estricto, parseo de cada ID, reconstrucción mediante validators reales de settings/bounds/`WorldTopology`/`MacroWorldPlan`, verificación de ambos hashes canónicos, membership del active sector y validación estructural de provenance antes de publicar. La evidencia de creación no se compara con el contenido actual y no decide generation/save compatibility.
...
- Esta shell no persiste gameplay/sector state, no aplica `CurrentSliceLoadService` y no completa geography, local detail ni la futura World Persistence de producción.
