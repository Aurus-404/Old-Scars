...
### Timed Bandaging V1 — shared actor treatment + NPC self-treatment

Fecha: 2026-09-04.

Estado: `VALIDATED — COMPLETED`.

`ActorWoundTreatmentController` reemplaza el commit instantáneo expuesto al Player por una operación de 4 segundos reales por actor. START fija wound + `ItemInstance` exactas sin mutar; COMPLETE revalida y usa el rollback transaccional existente. Caminar continúa; sprint o una acción fire/melee/reload del Player cancelan sin consumo. Health Window muestra progress/remaining y cerrarla no cancela. `DebugActionProgressController` conserva sus acciones previas y no se convirtió en autoridad médica.
...
### Blood Trails V1 — Medical bleeding distance trail
...
## 2026-09-07 — P2 Minimum real-time Unconscious dwell DONE / PUBLISHED

- Base: dev, HEAD/origin `0b21cf17463ed68a0ca01d1f9a87732ae186515d`. Implementación existente continuada sin reimplementar; commit funcional publicado `9ca0335cdc8b85bd49d20ddbe97ad814f44c8578`.
- Condition compartida Player/NPC conserva la autoridad. Gate sólo en Unconscious, una vez por episodio; Unity real time independiente de WorldClock. Physiology continúa; Death terminal atraviesa el gate; expirar no fuerza despertar ni genera ping-pong.
- Actor profile `minimum_unconscious_real_seconds`: Core 5 s inicial de prueba, no balance definitivo. Probado también 0.3 s configurable.
- Current Slice schema v1 aditivo: restante y continuidad del episodio, sin timestamps UTC. Capture conserva el último frame evaluado para comparación/rollback síncronos exactos. Load reanuda restante; legacy que deriva Unconscious recibe mínimo completo. No incremento de schema.
- Diagnostic P2 PASS: x1 5.001 s, x100 5.002 s, nueva lesión sin reinicio, fisiología, no forced wake, muerte por Vital Integrity y sangre, nuevo episodio, Player/NPC save/load, datos inválidos y rollback exacto. Segunda sesión automatizada de Play Mode: 4.000 s restantes preservados tras más de 6 s fuera de Play; recovery observado a 4.240 s desde iniciar la carga, incluyendo trabajo de restore. No se presenta como aceptación manual.
- Regresiones PASS/exit 0: Actor Consciousness/Incapacitation; Actor Physical Collapse; M39.0 Localized Health/Medicine (incluye rollback); M38.0 Actor Runtime/Lifecycle (incluye rollback); M41.1 Human Encounter AI; M41 Sandbox Preparation/Behavior ownership (`RunBatchWorldRuntime`); Timed Bandaging/NPC Self-Treatment. Se reutilizaron los PASS existentes al retomar; no batería global ni rerun de Search/Reload.
- Fixtures: Consciousness espera el mínimo y aísla hysteresis con episodio ya agotado; Collapse espera el restante restaurado y navega mediante Behavior ownership. Conservadas assertions de recuperación, postura y movimiento real; ISSUE-0024 tooling resuelto, sin cambiar gameplay. La primera invocación de ownership usó un nombre de entrada incorrecto; corregido sólo en el lanzamiento local.
- Revisión dedicada del diff P2 sin defectos adicionales de runtime/persistence; hallazgo de espera en collapse atendido. Revisión final de los fixtures sin relajar aceptación. Compile Runtime/Editor y git diff --check PASS; logs conservan warnings de paquetes/APIs fuera del cambio y fallos inyectados explícitos, no equivalen a Console manual limpia.
- ProjectSettings/ProjectSettings.asset user-owned byte-for-byte preservado: SHA-256 `034E692F54521A2491E132945191829F4BA6716778C04F881A63247D701DD51C`, fuera de staging/commits. Sólo ocho archivos funcionales P2 incluidos. Push funcional sincronizado con origin/dev, divergencia 0/0.
- ISSUE-0021 RESOLVED; IMPL-0015 DONE; P2 DONE/PUBLISHED. Próximo trabajo exacto: P3 KO / combat-memory continuity (ISSUE-0020 / IMPL-0014). P3 no iniciado. F8/aim, Invincible y Carry Weight sin cambios; aceptación manual integrada y cierre M41 siguen pendientes.

## 2026-09-07 — P3 KO / combat-memory continuity DONE / PUBLISHED

- P3 existente continuado sin reimplementar ni cambiar la política aprobada. Base inicial `8cdea394bd10c5075d95d4ebb867ec783afb2cf1`; integrado por fast-forward `6da975a9651d8e9e065c7a0d43c0120ff17b9ddb` (backlog de contenedores especializados), sin solapamiento y con hashes de todos los archivos locales preservados. IMPL-0022 permanece intacto.
- Commit funcional publicado `394d01886b8c6697ca2d492c4450282f561ba688`, siete archivos: Encounter, Definition, validator, Core actor profiles, diagnostic P3 + meta y fixture Human Encounter. Push dev confirmado, HEAD/origin iguales y divergencia 0/0.
- Causa corregida: liberar amenaza/acciones por KO también borraba el contexto reciente. Encounter ahora conserva una sola identidad/contexto efímero independiente de Threat, sin posición ni nueva autoridad. No añade MemorySystem, planner, reloj global, Search ni estado durable.
- `recent_enemy_memory_seconds`: 60 s Core como tuning inicial de prueba, no balance final. Tiempo real local, pausa por incapacidad propia, renovación por observaciones positivas legítimas; incapacidad oculta del rival no pausa el dueño. Expiry, death/invalidation, disable/reconfiguración y reemplazo limpian el recuerdo.
- KO libera Threat/acciones y no recibe ataques deliberados. Recovery visible u oculto requiere reconocimiento/percepción real para retomar Fighting con el mismo contexto, sin otro ciclo Alerted. La memoria no revela posición escondida ni dirige/prolonga Search. Treatment y AmbientTopOff siguen usando su ventana segura existente; EmptyWeapon conserva continuidad/cancelación/consumo.
- Diagnostic P3 PASS en Play Mode: combate automático, KO con rival cargado sin disparar, ambos recuerdos, treatment, AmbientTopOff con consumo exacto, recovery visible y oculto con reconocimiento gradual, ninguna posición nueva oculta, nuevo enemigo, death, runtime removal y expiración configurable pausada sólo por incapacidad propia. Assertions reforzadas verifican las transiciones reales, no sólo contadores.
- Ocho regresiones PASS/exit 0: P2 Unconscious Dwell (x1 5.001 s, x100 5.001 s; 3.999 s preservados tras más de 6 s fuera de Play, rollback/legacy), Human Encounter, Behavior ownership, LostContact/Search, Gaze/Perception, Timed Bandaging/NPC Self-Treatment, NPC Opportunistic Reload y M38 Actor Lifecycle/persistence. No batería global. P2 runtime/persistence permanece sin cambios; Core 5 s sigue siendo tuning provisional.
- Correcciones sólo de fixtures: el primer caso oculto movía al KO al borde fuera de NavMesh; corregidos punto y oclusor físico/carving para mantener oclusión real en ambos sentidos. Human Encounter esperaba Alerted después de Search ya liberada; ahora exige percepción/LKP positiva, mismo contexto, una reanudación y transición directa a Fighting. No se relajó aceptación ni se cambió gameplay fuera de P3.
...
## 2026-09-13 — M41.4 / P7 Player Debug Invincible DONE / PASS / PUBLISHED

- Objetivo: permitir QA prolongada del Player con el pipeline físico/médico real activo y bloquear únicamente el desenlace terminal `Dead`. Antes de P7 no existía el tooling y una salida ingenua antes de `ProcessDeath()` habría dejado Health, Condition, lifecycle, tags y Current Slice en contradicción.
- Caminos terminales auditados: `ApplyVitalDamage → ProcessDeath`; `ActorCondition.AdvancePhysiology → Health.Kill` por fatal blood; `DebugActionExecutor → Kill`; restore legacy de corpse → `Kill`; `ApplyInitialHealth/Awake → ProcessDeath`. Sólo Health publica lifecycle `Dead` y tags `dead_actor/lootable_actor`; no existe un Game Over/Player death consumer separado en el runtime actual.
- Implementación: `ActorDebugInvincible` es un marker efímero, target-side y sin estado serializado, creado sólo para el Player desde Runtime Debug Tools/F3 junto a `Invisible to AI`. Health conserva una Vital Integrity positiva de hasta `0.001` ante intentos terminales runtime protegidos, sin aumentarla. Condition sigue ejecutando bleeding y el intento productivo `Health.Kill`, mantiene Blood `0.0001` por encima de `fatalBloodFraction` para que Alive/snapshot sean consistentes, y permite Incapacitated/Unconscious, minimum real-time dwell y tratamiento. Un actor ya Dead no revive.
- OFF conserva gameplay normal. Apagar el toggle no cura, resetea, resucita ni mata dentro del setter; el siguiente daño o avance fisiológico fatal vuelve a `ProcessDeath`, lifecycle Dead y tags terminales. NPCs sin marker no reciben protección. No se modificaron AI, Perception, Combat, accuracy, armor, anatomy, hitboxes, KO tuning, persistence schema ni save version.
- Validación Unity `6000.4.6f1`, checkout/Library canónicos: Runtime compile `PASS` (`RUNTIME_EXIT=0`); Editor compile `PASS` (`EDITOR_EXIT=0`); `M41 Player Debug Invincible Diagnostics: PASS`; `Actor Consciousness & Incapacitation Diagnostics: PASS`. El primer run del diagnostic detectó que su fixture Edit Mode no agregaba Condition vía `Awake`; se corrigió el fixture sin cambiar aceptación. Review posterior eliminó una posible resurrección al repetir `Kill()` sobre un Dead protegido y el diagnostic principal volvió a PASS.
- P4 Prueba 3.3 se reconcilia como `DONE / ACCEPTED` por confirmación previa de Mauro; no se volvió a ejecutar ni se inventó evidencia. 30v30 y variedad anatómica de impactos continúan diferidos en ISSUE-0026/0027. `ISSUE-0012 RESOLVED`; `Implementation_Backlog.md` sin cambios.
- Commit funcional: `c96900589816239bfdf6553fba699bca6b79a54f`. Los cambios locales previos de MainMenu, ProjectSettings y combat telemetry permanecen preservados y fuera del stage.
- NEXT EXACT STEP: **P8 — completar Observability V2 / F10** sobre el tooling existente; P9 QA integrada/cleanup permanece después. F8D, 30v30 y anatomical hit distribution no se reactivan sin evidencia/tarea propia.

## 2026-09-13 — M41.4 / P8 / F10 Observability V2 DONE / ACCEPTED / PUBLISHED

- Objetivo: completar la observabilidad de targeting y disparos sobre el panel F6 existente para explicar combates sin inferir el comportamiento de AI a ojo.
- Implementación: overlay CURRENT compacto multi-NPC con afiliación, identidad ActorInstanceId abreviada, estado de encounter/AI, target y estado funcional; inspector del seleccionado con target/threat, distancia, CurrentAimPoint productivo, Primary Aim Point informativo, Focus, Spread efectivo y AttackCount.
- Último disparo real: snapshot acotado en el panel con secuencia, target previsto, LastShotOrigin/Direction, termination física, endpoint, collider, Combat.Region nullable, armor y clasificación ACTOR HIT/MISS/WORLD-OBSTACLE. BodyRegion sólo procede de Combat.Region; la región ausente se presenta NONE. CURRENT AIM y LAST SHOT se mantienen diferenciados.
- Semantic trace bounded a 12 entradas: transiciones de lifecycle/encounter/threat/Condition, KO/recovery y shot con clasificación y región; no hay logging por frame. El inspector conserva observabilidad Conscious/Incapacitated/Unconscious/Dead y dwell mediante getter existente.
- Autoridades tocadas: SandboxNpcObservabilityPanel y M41F6ObservabilityDiagnostics. El tooling es read-only y no añade autoridad de AI, Perception, Combat, Health, Condition ni persistence; usa los getters de AI ya publicados en dev.
- Validación automatizada previa a este cierre: Runtime compile PASS; Editor compile PASS; M41 F10 Observability Diagnostics PASS; M41 F6 Observability Diagnostics PASS; git diff --check del scope P8 PASS. Los gates Unity no se repitieron porque después de esos PASS sólo hubo cierre documental y separación de staging.
- Aceptación visual manual de Mauro: PASS el 2026-09-13. Blue y Red visibles a la vez; selección clara; CURRENT Gaze/FOV y CURRENT/LAST diferenciados; Conscious/Incapacitated/Dead; actor Dead sin targeting CURRENT; LAST REAL SHOT WORLD/OBSTACLE y world collider visibles; Combat.Region NONE en impacto no anatómico; semantic trace legible e inspector profundo navegable. El clutter ocasional de líneas se acepta para QA.
- Commits: funcional f4d07434d2b0ea0387584320c81b48dd248bd280; cierre documental separado en esta secuencia de publicación.
