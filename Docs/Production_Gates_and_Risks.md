...
- identity/topology/determinism, macro elevation/landforms, Macro Water y gameplay-quality globales validados; climate/rivers y demás features cross-sector coherentes siguen requeridos antes de realización local;
...
| R07 | `MITIGATING` | Mauro | Worldgen prematuro o acoplado irreversiblemente | Media | Alta | GameObjects o sectores independientes se generan antes de provenance, macro truth, determinismo y persistencia. | Provenance, identity/topology/determinism, session/save shell, Macro World Plan, Macro Elevation/Landforms y Macro Water/quality ya están validados como datos lógicos globales antes de GameObjects. La versión global del pipeline ya no re-seedea passes: cada pass posee contrato estable y dependencias explícitas. Create/Load/Runtime Ready emiten resúmenes estructurados de contracts/hashes/truth presente sin logging por celda/frame. Continuar sólo por passes autorizados, preservar truth compartida y no convertir el MST lógico en adjacency/travel física. | Revisar cada foundation; cerrar para el scope del Connected First Playable y reconfirmar en Production Ready. |
...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance está validada y `world_session_v1` schema 4 conserva MacroWorldPlan + geography + Water committed + evidencia de creación sin bloquear load; schemas 1/2/3 poseen compatibilidad delimitada sin fabricar truth posterior. `GeneratorVersion` global queda como metadata y los contratos de pass evitan drift upstream incidental. Faltan generation compatibility explícita y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
