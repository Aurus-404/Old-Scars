...
- identity/topology/determinism y macro elevation/landforms globales validados; hydrology/coastlines y demás features cross-sector coherentes siguen requeridos antes de realización local;
...
| R07 | `MITIGATING` | Mauro | Worldgen prematuro o acoplado irreversiblemente | Media | Alta | GameObjects o sectores independientes se generan antes de provenance, macro truth, determinismo y persistencia. | Provenance, identity/topology/determinism, session/save shell, Macro World Plan y Macro Elevation/Landforms ya están validados como datos lógicos globales antes de GameObjects. Continuar sólo por passes autorizados y preservar verdad macro compartida antes de detalle local. | Revisar cada foundation; cerrar para el scope del Connected First Playable y reconfirmar en Production Ready. |
...
| R11 | `OPEN` | Mauro | Rendimiento tardio | Media | Alta | Ningun escenario representativo tiene baseline. | Aprobar budgets y perfilar por etapa. Macro Elevation/Landforms registra timing plan+geography por preset (aprox. 14/49/199/910 ms en el diagnóstico actual); no son budgets productivos. El futuro worker budget será performance-only y 1/N workers deberá conservar output lógico idéntico. | Revisar en gates jugables y desde Content Pipeline; cerrar en Beta. |
...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance está validada y `world_session_v1` schema 3 conserva MacroWorldPlan + geography committed + evidencia de creación sin bloquear load; schemas 1/2 poseen compatibilidad delimitada sin fabricar geography. Faltan generation compatibility explícita y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
