...
| R07 | `MITIGATING` | Mauro | Worldgen prematuro o acoplado irreversiblemente | Media | Alta | GameObjects o sectores independientes se generan antes de provenance, macro truth, determinismo y persistencia. | Provenance, identity/topology/determinism, session/save shell, Macro World Plan, Macro Elevation/Landforms, Macro Water/quality y Macro Human Geography/Road Network ya están validados como datos lógicos globales antes de GameObjects. Human Geography posee contrato `macro_human_roads_v1`, no usa el MST lógico como road graph y no genera por sector. Create/Load/Runtime Ready emiten resúmenes estructurados sin logging por cell/road/frame. Continuar sólo por passes autorizados y preservar truth compartida. | Revisar cada foundation; cerrar para el scope del Connected First Playable y reconfirmar en Production Ready. |
