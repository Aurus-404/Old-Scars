...
- navegación/rendimiento medidos antes de producción sectorial; el spike aporta una baseline Terrain/NavMesh local, no el gate completo de partitions/streaming;
...
| R04 | `MITIGATING` | Mauro | Deuda OnGUI | Alta | Media | UI debug condiciona backends nuevos. | La application shell usa sólo OnGUI funcional mínimo sobre backends separados; congelar ampliaciones y reemplazar en M48.0. | Cerrar en Production Ready; aceptacion de deuda requiere decision explicita. |
...
| R07 | `MITIGATING` | Mauro | Worldgen prematuro o acoplado irreversiblemente | Media | Alta | GameObjects o sectores independientes se generan antes de provenance, macro truth, determinismo y persistencia. | Provenance, identity/topology/determinism, session/save shell, Macro World Plan, Macro Elevation/Landforms, Macro Water/quality y Macro Human Geography/Road Network ya están validados como datos lógicos globales antes de GameObjects. El Terrain Materialization Technical Spike consume schema 5 committed en una ventana local y mantiene `SectorId != Terrain tile`; no regenera truth, no persiste TerrainData y no genera por sector. Continuar sólo por units autorizados y preservar truth compartida. | Revisar cada foundation; cerrar para el scope del Connected First Playable y reconfirmar en Production Ready. |
...
| R11 | `OPEN` | Mauro | Rendimiento tardio | Media | Alta | Ningun escenario representativo tiene baseline. | Aprobar budgets y perfilar por etapa. Human Geography V1 registró generación/payload aprox. Small `28 ms/52 KB`, Medium `79 ms/98 KB`, Large `295 ms/160 KB`, Huge `1203 ms/288 KB`. El Terrain spike midió total `500/823/1295 ms` para `512/768/1024` Unity units; la baseline `768` dedicó `13 ms` a projection, `12 ms` a Terrain y `796 ms` a NavMesh. Son baselines técnicas, no budgets. Aplicar `precompute → persist → event-driven/low-frequency → continuous sólo con consumidor`, rechazar whole-world NavMesh y exigir output idéntico con 1/N workers. | Revisar en gates jugables y desde Content Pipeline; cerrar en Beta. |
...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance está validada y `world_session_v1` schema 5 conserva MacroWorldPlan + geography + Water + Human Geography committed + evidencia de creación sin bloquear load; schemas 1–4 poseen compatibilidad delimitada sin fabricar truth posterior ni silent upgrade. `GeneratorVersion` global queda como metadata y los contratos de pass evitan drift upstream incidental. Faltan generation compatibility explícita y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
