...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance está validada y `world_session_v1` schema 4 conserva MacroWorldPlan + geography + Water committed + evidencia de creación sin bloquear load; schemas 1/2/3 poseen compatibilidad delimitada sin fabricar truth posterior. Faltan generation compatibility explícita y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
