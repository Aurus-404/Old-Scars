...
| R07 | `MITIGATING` | Mauro | Worldgen prematuro o acoplado irreversiblemente | Media | Alta | GameObjects o sectores independientes se generan antes de provenance, macro truth, determinismo y persistencia. | Provenance, identity/topology/determinism, session/save shell y Macro World Plan V1 ya están validados como datos lógicos antes de GameObjects. Continuar sólo por passes autorizados y preservar verdad macro compartida antes de detalle local. | Revisar cada foundation; cerrar para el scope del Connected First Playable y reconfirmar en Production Ready. |
...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance está validada y `world_session_v1` schema 2 conserva settings resueltos, MacroWorldPlan y evidencia de creación sin bloquear load; schema 1 posee compatibilidad delimitada. Faltan generation compatibility explícita y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
