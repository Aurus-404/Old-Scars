...
| R20 | `MITIGATING` | Mauro | Incompatibilidad entre save, datos, assets, generación y mods | Media | Alta | Un cambio de definición/input rompe referencias, altera estado cargado o reinterpreta un sector no resuelto. | Source identity/provenance mínima ya validada; faltan generation compatibility explícita, persistencia del contrato, versiones/migrations y matriz de upgrades sin asumir igualdad de bytes como única semántica. | Revisar en cada foundation mundial; cerrar en Production Ready y revalidar en releases. |
